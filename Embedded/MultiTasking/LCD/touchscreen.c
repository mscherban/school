...
position unchanged. */
#define X_POS_FLAG  0x1
#define Y_POS_FLAG  0x2
uint32_t LCD_TS_Read(PT_T * position) {
	...
	if (b>0) {
		...
		return 0;
			...
			/* submit ADC request and wait for it to complete */
			DEBUG_TOGGLE(DBG_QUEUE_ADC_REQ);
			Qm.ChannelNum = LCD_TS_YU_CHANNEL;
			Qm.ResultVariable = &yu;
			Qm.EventId = f_Read_TS;
			Qm.ThreadFlag = X_POS_FLAG;
			osMessageQueuePut(ADCRQ, &Qm, 0, NULL);
			osEventFlagsWait(f_Read_TS, X_POS_FLAG, osFlagsWaitAll, osWaitForever);
			DEBUG_TOGGLE(DBG_QUEUE_ADC_REQ);
			x = yu;
			...
			osDelay(TS_DELAY);
					...
					/* submit ADC request and wait for it to complete */
			DEBUG_TOGGLE(DBG_QUEUE_ADC_REQ);
			Qm.ChannelNum = LCD_TS_XL_CHANNEL;
			Qm.ResultVariable = &xl;
			Qm.EventId = f_Read_TS;
			Qm.ThreadFlag = Y_POS_FLAG;
			osMessageQueuePut(ADCRQ, &Qm, 0, NULL);
			osEventFlagsWait(f_Read_TS, Y_POS_FLAG, osFlagsWaitAll, osWaitForever);
			DEBUG_TOGGLE(DBG_QUEUE_ADC_REQ);
			y = xl;
